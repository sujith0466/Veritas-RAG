...
class RetryPolicyEngine:
  ...
  # Backend: Redis + PostgreSQL write-through
```
...
Phase 6 Output (ConfidenceResultDTOv2)
    ...
    └─────────┬──────────┘
              │
    ┌─────────▼──────────┐
    │    RuleEngine       │ ← RetryPolicy (Redis cache)
    │  (rule evaluation)  │
    └─────────┬──────────┘
              │
    ┌─────────▼────────────────────────────────────┐
...
| `services/budget_manager.py` | NEW | `RetryBudgetManager` (Redis-backed) |
| `services/policy_engine.py` | NEW | `RetryPolicyEngine` (Redis+PostgreSQL) |
...
2. Daily budget is tracked per-tenant in Redis — prevents denial of service via retry flooding.
...
1. Budget checks use Redis `INCR` — O(1), sub-millisecond.
2. Policy lookup uses Redis read-through — O(1) cache hit.
...
- `RetryBudgetManager`: request budget enforced; daily budget enforced; Redis TTL behavior.
...
| `test_daily_budget_exhausted` | Redis daily budget limit hit → 429 |
...
| Budget Redis check | < 1ms | Redis latency |
...
|---|---|---|---|
| Agentic loop despite budget | Very Low | Critical | Hard cap 3 enforced server-side |
| Redis daily budget race condition | Low | Medium | Redis INCR is atomic |
| Policy rule priority conflict | Medium | Low | Validation at write time |
| Retry loop exceeds SLA budget | Medium | High | Phase 7 respects overall request timeout |
| Non-improving retry wastes resources | Medium | Medium | Monotonic check aborts early |
...
**Components**: `budget_manager.py` (Redis-backed), `retry_audit_logs` DB table (migration 0011).
