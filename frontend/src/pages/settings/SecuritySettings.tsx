...
export function SecuritySettings() {
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      ...
      toast({ title: 'Success', message: 'Password updated successfully. Please log in again.', type: 'success' })
      setFormData({ currentPassword: '', newPassword: '', confirmPassword: '' })
    } catch (error: any) {
      toast({ title: 'Error', message: error.response?.data?.error?.message || error.message || 'Failed to update password', type: 'error' })
    } finally {
      setSaving(false)
    }
  }
