  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      <motion.div
        ...
        className="bg-slate-900/60 backdrop-blur border border-slate-800/80 rounded-xl p-5 flex items-center justify-between shadow-lg"
      >
        <div>
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Total Chunks</p>
          <p className="text-2xl font-bold text-slate-100 mt-1">{totalChunks.toLocaleString()}</p>
          <p className="text-xs text-slate-500 mt-1">Across active documents</p>
        </div>
      ...
      >
        <div>
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Avg Token Density</p>
          <p className="text-2xl font-bold text-slate-100 mt-1">{avgTokens} <span className="text-xs font-normal text-slate-400">tokens</span></p>
          <p className="text-xs text-slate-500 mt-1">~{avgChars} chars per chunk</p>
        </div>
      ...
      >
        <div>
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Strategies Used</p>
          <div className="flex flex-wrap gap-1 mt-2">
            {Object.keys(breakdown).length === 0 ? (
              <span className="text-xs text-slate-500">None</span>
            ) : (
              Object.entries(breakdown).map(([strat, count]) => (
                <span key={strat} className="px-2 py-0.5 text-xs rounded-md bg-slate-800 border border-slate-700 text-slate-300 font-medium">
                  {strat}: {count}
                </span>
              ))
            )}
      ...
      >
        <div>
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Embedding Status</p>
          <p className="text-2xl font-bold text-amber-400 mt-1">{embeddedCount} <span className="text-xs font-normal text-slate-400">/ {totalChunks}</span></p>
