...
export function PayloadInspectorModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="relative w-full max-w-2xl rounded-xl border border-border bg-surface shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-border px-6 py-4 bg-background/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-semibold text-foreground">Vector Payload Inspector</h3>
          ...
          <button
            onClick={onClose}
            className="rounded-lg p-1 text-muted-foreground hover:bg-border hover:text-foreground transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs bg-background/50 border border-border/80 rounded-lg p-3.5">
            <div>
              <span className="text-muted-foreground block mb-1 flex items-center gap-1">
                <Database className="w-3 h-3 text-muted-foreground" /> Collection
              </span>
              <span className="font-mono text-foreground">{metadataRecord.collection_name}</span>
            </div>
            <div>
              <span className="text-muted-foreground block mb-1 flex items-center gap-1">
                <Hash className="w-3 h-3 text-muted-foreground" /> Points Count
              </span>
          ...
          <div className="space-y-1.5">
            ...
            <pre className="p-4 rounded-lg bg-background border border-border/80 text-xs font-mono text-indigo-200 overflow-x-auto leading-relaxed">
