...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        ...
        me_res = await client.get(API_URL + "/api/v1/auth/me", headers={"Authorization": f"Bearer {token}"})
        if me_res.status_code == 200 and me_res.json().get("data", {}).get("email") == "demo@gmail.com":
            print(" PASS")
        else:
