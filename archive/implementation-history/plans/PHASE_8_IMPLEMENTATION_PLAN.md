...
- HyDE uses the LLM provider (LLMProvider (Gemini Implementation)) to generate a hypothetical answer; falls back to template-based synthesis on failure.
...
| HyDE hypothetical doc misleads retrieval | Medium | Medium | Technical Lead | Confidence comparison before/after; rollback if worse | Graceful degradation |
| Decomposition creates too many sub-queries | Medium | Low | Technical Lead | Hard cap at 3; merge via FusionEngine | Graceful degradation |
| Entity recovery resolves to wrong entity | High | Medium | Technical Lead | Confidence comparison; original used if worse | Graceful degradation |
| LLM quota exceeded during heavy retry | High | Medium | Technical Lead | LLM call budget tracking per request | Graceful degradation |
| Expanded query too long for embedding | Low | Low | Technical Lead | Token budget cap on expanded query | Graceful degradation |
...
All third-party services utilized by the Query Rewrite Engine are abstracted through provider interfaces. This ensures no tight coupling to specific implementations (e.g., LLMProvider (OpenAI Implementation) or Google LLMProvider (Gemini Implementation)).
- `ILLMProvider`: For LLM-based decomposition, hyde text generation, and entity extraction.
- `ISynonymProvider`: For synonym and domain term expansion logic.

---

## 34. Architecture Decision Records (ADR)

- **ADR-P8-001**: Implement HyDE as a standard rewrite strategy rather than a hardcoded retrieval step.
- **ADR-P8-002**: Use heuristic fallbacks on LLM timeouts to guarantee < 200ms target response times.
- **ADR-P8-003**: Maintain stateful query rewrite audit logs for offline model improvement.

---

## 35. Versioning Strategy

- **API Versioning**: Endpoint remains at `/api/v1/query-rewrite` but introduces `RewriteRequestDTOv2` to maintain backward compatibility with Phase 3 payloads.
- **Data Versioning**: Audit log payloads support schema migration for varied JSON structures in `sub_queries` and `resolved_entities`.
...
Feature flags are configured in `QueryRewriteSettings` and optionally overrideable per-tenant via database records:
- `FF_REWRITE_HYDE_ENABLED`
- `FF_REWRITE_DECOMPOSITION_ENABLED`
- `FF_REWRITE_EXPANSION_ENABLED`
- `FF_REWRITE_ENTITY_RECOVERY_ENABLED`

---

## 37. Performance Budgets

- **Total Execution Budget**: 200ms per query rewrite.
- **LLM Call Budget**: 150ms maximum latency allowance for LLM providers.
- **Heuristic Overhead**: < 5ms for term lookups and string manipulations.
- **Database Write Overhead**: Async execution, not blocking the rewrite response.
...
```mermaid
sequenceDiagram
    participant RC as RetryController (P7)
    participant RO as RewriteOrchestrator
    participant SS as StrategySelector
    participant RS as RewriteStrategy
    participant LLM as LLM Provider
    participant Log as RewriteRepository

    RC->>RO: rewrite(request)
    RO->>SS: select_strategy(signals)
    SS-->>RO: selected_strategy
    RO->>RS: rewrite(query)
    alt LLM strategy (HyDE/Decomp)
        RS->>LLM: generate()
        LLM-->>RS: completion
    else Local strategy (Expansion)
        RS->>RS: local_compute()
    end
    RS-->>RO: RewriteResultDTO
    RO->>Log: log_rewrite() (async)
    RO-->>RC: RewriteResultDTO
```

---

## 39. Failure Recovery Matrix

| Failure Mode | Detection | Fallback Strategy | Alerting |
|---|---|---|---|
| LLM Provider Timeout | `TimeoutError` > 150ms | Heuristic rule template | Warning |
| LLM Quota Exceeded | 429 Status | Bypass Rewrite / Original Query | Critical |
| Strategy Parsing Error | JSON Decode Error | Return original query | Warning |
| DB Audit Log Failure | Exception in async task | Drop log | Silent / Metric |
...
```mermaid
graph TD
    A[Retry Controller P7] --> B[Rewrite Orchestrator P8]
    B --> C[Strategy Selector]
    C --> D[HyDE Strategy]
    C --> E[Expansion Strategy]
    C --> F[Decomposition Strategy]
    C --> G[Entity Recovery Strategy]
    B --> H[Retrieval Orchestrator P5]
    B --> I[Rewrite Audit DB]
```
...
- **Immediate Mitigation**: Disable feature flags (e.g. `QUERY_REWRITE_HYDE_ENABLED=false`).
- **Full Rollback**: Revert to the legacy `ClarificationEngine.rewrite_query()` provided in Phase 3.
- **Data Retention**: Dropping `query_rewrite_logs` is not required for a rollback, data remains for offline analysis.

---

## 42. Success Metrics

- **System Reliability**: >99.9% of rewrites complete within the 200ms performance budget.
- **Semantic Quality**: >15% improvement in overall retrieval confidence scores post-rewrite.
- **Recovery Success**: >95% LLM timeouts gracefully fall back to heuristic templates without errors.
- **Audit Completeness**: 100% of rewrites logged asynchronously to the DB.
