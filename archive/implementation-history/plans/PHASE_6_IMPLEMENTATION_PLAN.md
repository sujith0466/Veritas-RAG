...
class ConfidencePolicyStore:
  ...
  # Backend: CacheProvider (Redis Implementation) (fast) + PostgreSQL (durable) with write-through caching
```
...
1. `ConfidencePolicyStore` uses CacheProvider (Redis Implementation) read-through cache (< 1ms lookup) backed by PostgreSQL.
...
| Policy store cache hit | < 1ms | CacheProvider (Redis Implementation) latency |
...
| NLI-based conflict detection too slow | Medium | Medium | Technical Lead | Default to heuristic provider; NLI optional | Graceful degradation |
| Policy weight misconfiguration causes wrong routing | High | Medium | Technical Lead | Pydantic constraints + weight sum validation | Graceful degradation |
| Coverage analyzer misses domain-specific clauses | Medium | Medium | Technical Lead | Configurable clause splitter patterns per tenant | Graceful degradation |
| ConfidenceEvaluationLog write failures degrade performance | Low | Low | Technical Lead | Fire-and-forget async; failures logged but ignored | Graceful degradation |
| FreshnessAnalyzer fails on missing document dates | Medium | Low | Technical Lead | Default to neutral score (0.5) on missing dates | Graceful degradation |
...
- [ ] Per-tenant `ConfidencePolicy` is applied within < 2ms (CacheProvider (Redis Implementation) cache).
...
The Confidence Engine abstracts the NLI (Natural Language Inference) provider to decouple conflict detection from specific vendor implementations.
- `BaseNLIProvider`: Abstract interface for entailment checking.
- `HeuristicNLIProvider`: Default fallback using keyword/antonym detection without network calls.
- `CohereNLIProvider`: Future implementation leveraging external LLM classification.
- `LocalNLIProvider`: Future implementation leveraging local transformer models (e.g. DeBERTa).

## 38. Architecture Decision Records (ADR)

- **ADR-P6-001: Confidence Formula Matrix**: Defines the 4-signal weighted approach to confidence scoring rather than a monolithic LLM-as-a-judge, improving latency and explainability.
- **ADR-P6-002: Conflict Detection Entailment**: Defines the fallback strategy from NLI (Natural Language Inference) to heuristic keyword antonyms to guarantee the < 50ms latency budget.
- **ADR-P6-003: Freshness Decay Curves**: Adopts pluggable decay functions (linear, exponential, step) over strict cutoff dates to allow nuanced scoring for domain-specific temporal documents.

## 39. Versioning Strategy

- **API Versioning**: Endpoints mounted under `/api/v1/confidence/`. Breaking changes to the evaluation endpoint will roll to `v2`.
- **DTO Versioning**: Using `ConfidenceResultDTOv2` to maintain backward compatibility with Phase 3 stub consumers.
- **Database Migrations**: Alembic migration `0010` is strictly additive.
- **Internal Event Versioning**: `CONFIDENCE_EVALUATED` event schema versioned with `schema_version = "1.0"`.
...
- `FF_ENABLE_CONFIDENCE_ENGINE` (default: `True`): Master toggle for the entire evaluation pipeline. If false, bypasses straight to Phase 10 with a default score of 100.
- `FF_ENABLE_NLI_CONFLICT_DETECTION` (default: `False`): Toggles external NLI inference. Defaults to False for performance.
- `FF_ENABLE_DEGRADED_THRESHOLDS` (default: `True`): Dynamically adjusts PROCEED thresholds upward if upstream retrieval was in a degraded state.

## 41. Performance Budgets

- **Total Latency**: P95 < 50ms for k=10 evidence chunks.
- **Memory**: Max 50MB per evaluation request for in-memory token matrices.
- **CPU**: Max 20ms of compute per request (async offloading for non-blocking I/O).
- **LLM/NLI Calls**: 0 synchronous external network calls in the hot path unless `FF_ENABLE_NLI_CONFLICT_DETECTION` is True (adds ~150ms budget).
...
sequenceDiagram
    participant Client
    participant API as /api/v1/confidence
    participant CE as ConfidenceEngine
    participant Analyzers as 4x Sub-Analyzers
    participant DB as PolicyStore & Repo

    Client->>API: POST /evaluate (RetrievalResult)
    API->>DB: GET ConfidencePolicy (tenant)
    API->>CE: evaluate(request, policy)
    par Concurrent Analysis
        CE->>Analyzers: CoverageAnalyzer.analyze()
        CE->>Analyzers: EvidenceStrength.score()
        CE->>Analyzers: FreshnessAnalyzer.analyze()
        CE->>Analyzers: ConflictDetector.analyze()
    end
    Analyzers-->>CE: 4x Signal Reports
    CE->>CE: aggregate_weighted_score()
    CE->>CE: resolve_action(score, thresholds)
    CE->>DB: async log_evaluation()
    CE-->>API: ConfidenceResultDTOv2
    API-->>Client: Result + PROCEED/RETRY action
```
...
|---|---|---|---|---|
| NLI Provider Timeout | Request > 50ms | `HeuristicNLIProvider` | Transient failure ignored | Log `WARNING`, alert if > 5% / hr |
| Policy DB Unavailable | Connection Refused | In-memory Default Policy | Auto-reconnect via pool | Log `ERROR`, alert immediate |
| Audit DB Unavailable | Insert Timeout | Skip audit logging | Async retry queue | Log `ERROR`, alert queue depth |
| Clause Extractor Failure | Regex recursion | Fallback to full-query token overlap | Discard bad clause | Log `WARNING` |

## 44. Dependency Graph

- **Upstream Modules**: Retrieval Engine (Phase 5) -> Provides `RetrievalResultDTOv2`.
- **Downstream Modules**: Retry Controller (Phase 7), Query Rewrite (Phase 8), Clarification (Phase 9) -> Consume `ConfidenceResultDTOv2`.
- **Runtime Dependencies**: CacheProvider (Redis Implementation) (Policy Caching), PostgreSQL (Audit Logging).
- **External Providers**: Optional NLI Provider (e.g., RerankerProvider (Cohere Implementation), local transformers).

## 45. Rollback Strategy

- **API Rollback**: Revert routing to Phase 3 `ConfidenceEngine` stub via environment variable `CONFIDENCE_ROUTING_VERSION=v1`.
- **Database Rollback**: Execute Alembic downgrade `0009`. Data in `confidence_evaluation_logs` is safe to truncate on rollback.
- **Feature Flag Rollback**: Instantly disable `FF_ENABLE_CONFIDENCE_ENGINE` via dynamic config injection (no deployment required).
- **Emergency Disable**: Returns default `ConfidenceResultDTOv2` with `PROCEED` action to ensure downstream generation is unblocked.

## 46. Success Metrics

- **Confidence Calibration Rate**: Correlation between `ConfidenceScore` and user thumbs up/down > 0.8.
- **False Positive Rate**: < 2% of `PROCEED` actions result in LLM hallucinations (measured via Phase 4 Evals).
- **False Negative Rate**: < 5% of `ABORT` or `RETRY` actions had sufficient context to answer.
- **Conflict Detection Accuracy**: > 95% accuracy against golden contradiction dataset.
- **Latency**: 99% of evaluations complete in < 50ms.
...
6. `ConfidencePolicyStore` with CacheProvider (Redis Implementation) caching.
