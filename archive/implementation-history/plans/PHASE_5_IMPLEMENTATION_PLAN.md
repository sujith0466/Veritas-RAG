...
- BM25 index storage backed by CacheProvider (Redis Implementation) (in-memory) with periodic PostgreSQL snapshots for durability.
...
|---|---|
| Dense Retrieval (VectorProvider (Qdrant Implementation) HNSW) | ✅ |
...
| BM25 Index CacheProvider (Redis Implementation) Persistence | ✅ |
...
- Aligns with **VectorProvider (Qdrant Implementation) Vector Database** (ADR-004): INT8 quantization, payload filter indexes.
...
| Phase 2 | VectorProvider (Qdrant Implementation) `VectorIndexMetadata` | Collection name resolution |
...
| `BM25SparseSearchProvider` | `backend/modules/retrieval/providers/sparse/` | Phase 5 adds CacheProvider (Redis Implementation) persistence |
...
- `RetrievalOrchestrator.execute_hybrid_search()`: Wire `FilterDSL` → VectorProvider (Qdrant Implementation) payload filters.
- `BM25SparseSearchProvider`: Add CacheProvider (Redis Implementation)-backed index persistence.
...
│  │  VectorProvider (Qdrant Implementation) HNSW │  │ BM25 + CacheProvider (Redis Implementation) │  │   k=60 configurable   │  │
...
FilterDSL is compiled to VectorProvider (Qdrant Implementation) `Filter` objects at the `RetrievalOrchestrator` boundary. `tenant_id` is always injected server-side from JWT claims — never from client request body.
...
|---|---|---|
| `FilterDSLCompiler` | Translate `FilterDSL` → VectorProvider (Qdrant Implementation) filter objects with tenant enforcement | Service |
| `DenseRetrievalStage` | Embed query + search VectorProvider (Qdrant Implementation) HNSW + normalize scores | Service |
| `SparseRetrievalStage` | BM25 search + CacheProvider (Redis Implementation) index persistence + incremental updates | Service |
...
(VectorProvider (Qdrant Implementation) + OTel)       (BM25+CacheProvider (Redis Implementation) + OTel)     │
        │                  │                   │
        └──────asyncio.gather───────────────────┘
                          │
                          ▼
...
│   │   ├── bm25_provider.py       [MODIFY] add CacheProvider (Redis Implementation) persistence
...
| `services/filter_dsl_compiler.py` | NEW | `FilterDSLCompiler` — translate to VectorProvider (Qdrant Implementation) filter objects |
...
2. **Filter Injection Prevention**: `FilterDSL` fields are strictly typed Pydantic models. No raw dict pass-through to VectorProvider (Qdrant Implementation).
...
4. BM25 indexes are in-memory in CacheProvider (Redis Implementation) (O(1) load). Disk snapshots are async background tasks.
...
2. BM25 CacheProvider (Redis Implementation) index is per-tenant namespace — scales by adding CacheProvider (Redis Implementation) cluster shards.
3. VectorProvider (Qdrant Implementation) collection sharding follows VectorProvider (Qdrant Implementation)'s native horizontal scaling model.
...
| RET_004 | `VectorStoreUnavailableError` | 503 | VectorProvider (Qdrant Implementation) unavailable |
...
### Integration Tests
- `POST /api/v1/retrieval/search` with FilterDSL (mock VectorProvider (Qdrant Implementation)).
...
- BM25 CacheProvider (Redis Implementation) index load time < 10ms for tenant with 100K chunks.
...
| BM25 index CacheProvider (Redis Implementation) load | < 10ms | `raguard_retrieval_sparse_duration_seconds` |
...
| BM25 CacheProvider (Redis Implementation) index memory overflow for large tenants | High | Medium | Technical Lead | TTL eviction + async PostgreSQL snapshots | Graceful degradation |
| Cross-encoder timeout causing P95 > 400ms | Medium | Medium | Technical Lead | Hard timeout + RRF fallback (RET_006) | Graceful degradation |
| FilterDSL complexity explosion from client | Medium | Low | Technical Lead | Strict Pydantic model limits depth and fields | Graceful degradation |
| Semantic dedup false positives remove valid evidence | High | Low | Technical Lead | Configurable threshold + disable flag per request | Graceful degradation |
| Context compression destroys critical context | High | Low | Technical Lead | Min relevance threshold + length guard + fallback to raw | Graceful degradation |
...
- [ ] All unit tests pass with >90% code coverage with ≥90% coverage on new components.
...
**Acceptance**: FilterDSL compiles to valid VectorProvider (Qdrant Implementation) filter; tenant enforcement proven by test.
...
### Milestone 5.3 — BM25 CacheProvider (Redis Implementation) Persistence
**Objective**: Persist BM25 index to CacheProvider (Redis Implementation) with incremental updates.
**Scope**: Extend `BM25SparseSearchProvider` with CacheProvider (Redis Implementation) persistence.
**Components**: `providers/sparse/bm25_provider.py` (extended).
**Database Impact**: None (CacheProvider (Redis Implementation) only).
**Testing**: `TestSparseRetrievalRedis` (3 tests).
**Acceptance**: BM25 index survives process restart via CacheProvider (Redis Implementation); incremental update adds new chunks.
...
- **Vector Search Provider**: Abstracted via `BaseDenseRetrievalProvider`, allowing seamless switching between VectorProvider (Qdrant Implementation), VectorProvider (Pinecone Implementation), or VectorProvider (Milvus Implementation).
- **Sparse Search Provider**: Abstracted via `BM25SparseSearchProvider` interface with CacheProvider (Redis Implementation) integration for state, enabling alternative implementations like Elasticsearch.
- **Reranker Provider**: Generalized through `BaseRerankerProvider` to support RerankerProvider (Cohere Implementation), local Cross-Encoders, or other APIs without business logic changes.

---

## 40. Architecture Decision Records (ADR)

- **ADR-P5-001 (FilterDSL Framework)**: Define a strict Pydantic-based DSL for metadata filters instead of passing raw dicts to prevent injection and enforce tenant scoping.
- **ADR-P5-002 (Context Compression Strategy)**: Use TF-IDF guided extraction with an LLM-fallback over arbitrary truncation to preserve maximum semantic value within the token budget.
- **ADR-P5-003 (3-Phase Deduplication)**: Implement SHA-256 for exact matches, Token Jaccard for near-duplicates, and Semantic cosine similarity as a final sieve to guarantee clean context downstream.

---

## 41. Versioning Strategy

- **API Versioning**: Phase 5 endpoints extend the `v1` namespace (e.g. `/api/v1/retrieval/search`), utilizing `RetrievalResultDTOv2` for backward-compatible response upgrades.
- **Index Versioning**: VectorProvider (Qdrant Implementation) collections and CacheProvider (Redis Implementation) BM25 indexes are tagged with `_v2` suffixes, allowing zero-downtime migration from the Phase 2 schema.
...
|---|---|---|
| `FF_FF_ENABLE_CONTEXT_COMPRESSION` | Toggles the ContextCompressor stage on/off. | `true` |
| `FF_FF_ENABLE_SEMANTIC_DEDUP` | Toggles the expensive semantic dedup phase. | `false` |
| `RERANKER_TIMEOUT_FALLBACK` | Allows falling back to RRF if cross-encoder times out. | `true` |
| `FF_USE_LOCAL_RERANKER` | Forces the local HuggingFace cross-encoder instead of RerankerProvider (Cohere Implementation). | `false` |

---

## 43. Performance Budgets

| Stage | P50 Budget (ms) | P95 Budget (ms) | Hard Timeout (ms) |
|---|---|---|---|
| Dense Retrieval | <20 | <50 | 100 |
| Sparse Retrieval | <15 | <30 | 50 |
| RRF Fusion | <2 | <5 | 10 |
| Deduplication | <5 | <10 | 25 |
| Cross-Encoder Reranking | <50 | <150 | 2000 |
| Context Compression | <25 | <75 | 500 |
| **Total Pipeline** | **<117** | **<320** | **400** |
...
sequenceDiagram
    participant Client
    participant API as /search (v2)
    participant Orchestrator
    participant Dense as VectorProvider (Qdrant Implementation) (Dense)
    participant Sparse as CacheProvider (Redis Implementation) (Sparse)
    participant Dedup as DedupEngine
    participant Reranker as CrossEncoder

    Client->>API: POST /search with FilterDSL
    API->>Orchestrator: execute_hybrid_search()
    par Dense + Sparse Search
        Orchestrator->>Dense: search_qdrant()
        Orchestrator->>Sparse: bm25_search()
    end
    Dense-->>Orchestrator: CandidatePointDTO[]
    Sparse-->>Orchestrator: CandidatePointDTO[]

    Orchestrator->>Dedup: full_dedup_pipeline()
    Dedup-->>Orchestrator: RankedEvidenceDTO[]

    Orchestrator->>Reranker: rerank_batched(timeout=2s)
    alt Reranker Success
        Reranker-->>Orchestrator: RankedEvidenceDTO[]
    else Reranker Timeout
        Reranker--xOrchestrator: TimeoutException
        Orchestrator->>Orchestrator: Fallback to RRF
    end

    Orchestrator-->>API: RetrievalResultDTOv2
    API-->>Client: 200 OK
```

---

## 45. Failure Recovery Matrix

| Component Failure | Detection | Recovery Strategy | Degradation State |
|---|---|---|---|
| VectorProvider (Qdrant Implementation) Unreachable | Timeout/ConnectionRefused | Circuit breaker open | Sparse-only results |
| CacheProvider (Redis Implementation) Unreachable | Timeout/ConnectionRefused | Circuit breaker open | Dense-only results |
| RerankerProvider (Cohere Implementation) API Timeout | >2000ms response | Catch timeout exception | Fallback to RRF ordering |
| LLM Compressor Failure | 5xx / Timeout | Catch exception | Return uncompressed text |
...
```mermaid
graph TD
    A[RetrievalOrchestrator] --> B[FilterDSLCompiler]
    A --> C[DenseRetrievalStage]
    A --> D[SparseRetrievalStage]
    A --> E[FusionEngine]
    A --> F[DedupEngine]
    A --> G[CrossEncoderReranker]
    A --> H[ContextCompressor]

    C --> I[(VectorProvider (Qdrant Implementation) Vector DB)]
    D --> J[(CacheProvider (Redis Implementation) BM25 Index)]
    G --> K[RerankerProvider (Cohere Implementation) API / Local Model]
    H --> L[LLM Inference / TF-IDF]
```
...
- **API Rollback**: Revert routing configuration to point to `RetrievalOrchestrator.execute_hybrid_search()` (v1) instead of v2.
- **Index Rollback**: Retain the `_v1` VectorProvider (Qdrant Implementation) collections and BM25 index snapshots for at least 7 days post-deployment.
- **Database Rollback**: Execute Alembic downgrade `0009_retrieval_v2_schema` to remove `retrieval_query_logs` extensions if needed.

---

## 48. Success Metrics

- **Quality**: Increase in Precision@5 by at least 15% compared to Phase 2 baseline.
- **Performance**: P95 retrieval latency remains under 400ms despite added reranking and compression stages.
- **Efficiency**: Downstream token usage reduced by ≥30% due to Context Compressor implementation.
- **Reliability**: 0 cross-tenant data leaks identified in automated penetration testing or manual review.
...
- [ ] Modify `backend/modules/retrieval/providers/sparse/bm25_provider.py` (CacheProvider (Redis Implementation) persistence)
...
5. CacheProvider (Redis Implementation)-backed BM25 persistence.
