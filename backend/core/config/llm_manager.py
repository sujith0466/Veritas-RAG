...
class LLMManagerSettings(BaseSettings):
    ...
    provider_priority_raw: str = Field(default="openrouter,gemini", alias="LLM_PROVIDER_PRIORITY")
    ...
    def priority_list(self) -> list[str]:
        ...
        if not self.provider_priority_raw:
            ...
            return [p.strip().lower() for p in (self.primary_provider, self.fallback_provider) if p.strip()]
        providers = [p.strip().lower() for p in self.provider_priority_raw.split(",") if p.strip()]
