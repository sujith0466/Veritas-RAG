...
class ProcessingJobService:
    ...
    async def claim_job(
        ...
        async with acquire_lock(self.redis, lock_key, timeout_sec=60, wait_sec=2) as locked:
            if not locked:
    ...
    async def complete_step(
        ...
        await self.audit_repo.append(
            job_id, "STEP_COMPLETED", worker_id, {"step_name": step_name, "duration_ms": duration_ms, "metrics": metrics}, session
        )
    ...
    async def record_step_error(
        ...
        """Record an error for a step. If fatal or max retries reached, move to DLQ."""
        step = await self.step_repo.get_step(job_id, step_name, session)
        if step:
            await self.step_repo.update_step_status(
        ...
        if is_fatal or job.retry_count >= job.max_retries:
            ...
            await self.audit_repo.append(job_id, "JOB_DLQ", worker_id, {"reason": job.dlq_reason}, session)
        else:
            job.status = "QUEUED"
            await self.audit_repo.append(job_id, "STEP_FAILED_RETRY", worker_id, {"step": step_name, "error": error_message}, session)
    ...
    async def mark_job_completed(
        ...
        """Mark the overall job as fully completed."""
        job = await self.job_repo.get_by_id(job_id, session)
        if not job:
            return
    ...
    async def requeue_stale_jobs(
        ...
        for job in stale_jobs:
            # Check redis if worker is still heartbeating
            ...
            if not is_locked:
                ...
                job.claimed_at = None
                await self.audit_repo.append(job.id, "JOB_REQUEUED_STALE", "system_cron", None, session)
