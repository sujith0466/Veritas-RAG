...
from backend.services.folder.events import FolderChildrenSoftDeletedEvent, FolderChildrenRestoredEvent
...
async def _hard_delete_folder_subtree(self, folder_id: str, workspace_id: str):
    ...
    try:
        ...
        if self.request.retries >= self.max_retries:
            await redis_client.lpush("folder:purge:dlq", f"{folder_id}:{workspace_id}:{str(e)}")
        raise self.retry(exc=e)
    finally:
