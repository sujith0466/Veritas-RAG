...
class ClaimExtractionFailed(ReflectionDomainException):
    def __init__(self, message: str = "Failed to extract claims from answer", detail: dict | None = None):
        super().__init__(message=message, error_code=ReflectionErrorCode.CLAIM_EXTRACTION_FAILED, detail=detail)


class ValidationFailed(ReflectionDomainException):
    def __init__(self, message: str = "Claim validation failed", detail: dict | None = None):
        super().__init__(message=message, error_code=ReflectionErrorCode.VALIDATION_FAILED, detail=detail)
