...
class ConfidenceEngine:
    ...
    def evaluate(self, request: ConfidenceEvalRequestDTO) -> ConfidenceResultDTO:
        ...
        # 1. Run all scorers
        coverage: CoverageMetricsDTO = self.coverage_analyzer.analyze(
            request.query, candidates
        )
        contradiction: ContradictionReportDTO = self.contradiction_detector.analyze(
            candidates
        )
        ...
        if (
            retrieval_result.is_degraded_fallback
            or retrieval_result.is_zero_result_broadened
        ):
        ...
        elif coverage.coverage_score < 0.2 and contradiction.contradiction_score < 0.2:
            action = ConfidenceAction.CLARIFY
        else:
            action = ConfidenceAction.ABORT

        logger.info(
            f"ConfidenceEngine computed score={score:.2f} action={action} for query_id={retrieval_result.correlation_id}"
        )
