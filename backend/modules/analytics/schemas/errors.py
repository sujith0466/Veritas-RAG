...
class RecordNotFound(AnalyticsDomainException):
    def __init__(self, message: str = "Analytics query record not found", detail: dict | None = None):
        super().__init__(message=message, error_code=AnalyticsErrorCode.RECORD_NOT_FOUND, detail=detail)


class InvalidDateRange(AnalyticsDomainException):
    def __init__(self, message: str = "Invalid date range or interval specified for query analytics", detail: dict | None = None):
        super().__init__(message=message, error_code=AnalyticsErrorCode.INVALID_DATE_RANGE, detail=detail)
