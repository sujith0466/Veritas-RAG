...
class RewriteOrchestrator:
    ...
    def __init__(
        ...
        if not strategy_selector:
            strategies = {
                ...
                RewriteStrategy.DECOMPOSITION: QueryDecompositionStrategy(llm_provider=llm_provider),
                RewriteStrategy.ENTITY_RECOVERY: MissingEntityRecoveryStrategy(llm_provider=llm_provider),
            }
    ...
    def rewrite(self, request: RewriteRequestDTOv2) -> RewriteResultDTO:
        ...
        logger.info("RewriteOrchestrator processing rewrite request", query=request.original_query, hint=request.strategy_hint)
        strategy = self.strategy_selector.select(request)
        
        result = strategy.rewrite(request)
        self._audit_log.append(result)
        logger.info("Rewrite complete", strategy=result.strategy, rewritten=result.rewritten_query)
        return result
