...
class StrategySelector:
    ...
    def select(self, request: RewriteRequestDTOv2) -> BaseRewriteStrategy:
        ...
        if request.strategy_hint and request.strategy_hint != RewriteStrategy.AUTO:
            ...
            logger.warning("Requested strategy hint not registered, falling back to signal routing", hint=request.strategy_hint)
        ...
        if (tokens & pronouns or "the above" in query_lower) and request.conversation_history:
            if RewriteStrategy.ENTITY_RECOVERY in self.strategies:
                logger.info("Selecting ENTITY_RECOVERY based on pronoun/reference detection")
                return self.strategies[RewriteStrategy.ENTITY_RECOVERY]

        # Check for complex/multi-part query signals
        if (len(query_lower.split()) > 20 or "?" in query_lower[:-1] or " and " in query_lower or "compare" in query_lower):
        ...
        if request.uncovered_clauses or (request.coverage_score is not None and 0.2 < request.coverage_score < 0.6):
            if RewriteStrategy.EXPANSION in self.strategies:
                logger.info("Selecting EXPANSION based on uncovered clauses / partial coverage")
