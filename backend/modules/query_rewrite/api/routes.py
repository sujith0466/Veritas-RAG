...
@router.post(
    "/rewrite",
    response_model=RewriteResultDTO,
    summary="Rewrite query using optimal strategy",
)
...
@router.get(
    "/strategies", response_model=list[str], summary="List available rewrite strategies"
)
...
@router.get(
    "/history",
    response_model=list[RewriteResultDTO],
    summary="Get recent rewrite history",
)
...
@router.post(
    "/clarify/evaluate",
    response_model=Optional[ClarificationQuestionDTO],
    summary="Evaluate if query requires clarification",
)
async def evaluate_clarification(
    request: RewriteRequestDTOv2, correlation_id: str
) -> ClarificationQuestionDTO | None:
    """Check if query requires user clarification and record paused state if needed."""
    return await _clarification_engine.evaluate_and_clarify(
...
@router.post(
    "/clarify/resume",
    response_model=ClarifiedQueryDTO,
    summary="Resume paused execution with user clarification choice",
)
...
@router.get(
    "/clarify/state/{correlation_id}",
    response_model=Optional[ClarificationStateDTO],
    summary="Get active clarification state",
)
async def get_clarification_state(correlation_id: str) -> ClarificationStateDTO | None:
