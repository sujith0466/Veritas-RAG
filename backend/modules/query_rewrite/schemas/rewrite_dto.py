...
class RewriteRequestDTOv2(BaseModel):
    ...
    conversation_history: list[str] = Field(default_factory=list, description="Prior turns for entity resolution")
    domain: str | None = Field(None, description="Domain hint for synonym expansion (e.g. 'finance', 'legal')")
    coverage_score: float | None = Field(None, ge=0.0, le=1.0, description="Coverage score from ConfidenceEngine")
...
class RewriteResultDTO(BaseModel):
    ...
    rationale: str
    sub_queries: list[str] = Field(default_factory=list, description="Non-empty only for DECOMPOSITION strategy")
...
class ClarificationStateDTO(BaseModel):
    ...
    question_text: str = Field(..., description="Clarification question presented to user")
    options: list[str] = Field(default_factory=list, description="Presented multiple choice options")
    status: ClarificationStatus = Field(default=ClarificationStatus.REQUIRED)
    created_at: float = Field(..., description="Timestamp when clarification was requested")
    expires_at: float = Field(..., description="Timestamp when clarification request expires")
    selected_option: str | None = Field(default=None, description="User selected resolution option")
    clarified_query: str | None = Field(default=None, description="Final resolved query string after clarification")
    model_config = ConfigDict(from_attributes=True)


class ClarificationResumeRequestDTO(BaseModel):
    """Payload submitted by user to resume execution after clarification."""
    correlation_id: str = Field(..., description="Unique request tracking ID corresponding to paused state")
    tenant_id: str = Field(..., description="Tenant namespace ID")
    selected_option: str = Field(..., min_length=1, description="Option selected or free-text clarification from user")
    additional_context: str | None = Field(default=None, description="Optional extra context provided by user")
    model_config = ConfigDict(from_attributes=True)
