...
class KnowledgeHealthScanCompletedPayload(BaseModel):
    ...
    parity_status: str = Field(
        default="UNKNOWN", description="Count parity status string."
    )
...
class OrphanChunksPurgedPayload(BaseModel):
    ...
    document_id: UUID | None = Field(
        default=None, description="Parent document ID if explicit purge."
    )
    ...
    reason: str = Field(
        ...,
        description="Trigger explanation (e.g. 'TWO_PHASE_PURGE', 'SCHEDULED_ORPHAN_SWEEP').",
    )
...
class KnowledgeDriftDetectedPayload(BaseModel):
    ...
    drift_type: str = Field(
        ...,
        description="Drift classification (`PARITY_MISMATCH` or `MODEL_ROTATION_STALE`).",
    )
    details: dict[str, Any] = Field(
        default_factory=dict, description="Drift metrics and counts."
    )
