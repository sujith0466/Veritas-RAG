...
async def _async_run_orphan_sweep(tenant_id: str) -> Dict[str, Any]:
    ...
    async with session_factory() as session:
        ...
        job_dto = await service.run_health_scan(tenant_id=tenant_id, scan_type=ScanType.ORPHAN_SWEEP)
        logger.info("Completed scheduled orphan sweep task", tenant_id=tenant_id, purged=job_dto.orphans_purged)
...
async def _async_run_parity_audit(tenant_id: str) -> Dict[str, Any]:
    ...
    async with session_factory() as session:
        service = KnowledgeHealthOrchestrator(session)
        job_dto = await service.run_health_scan(tenant_id=tenant_id, scan_type=ScanType.PARITY_AUDIT)
        logger.info("Completed scheduled parity audit task", tenant_id=tenant_id, parity=job_dto.parity_status)
        return job_dto.model_dump()


@celery_app.task(bind=True, queue="ingestion", max_retries=3, acks_late=True)
def execute_hard_purge_task(self: Any, document_id: str, tenant_id: str) -> Dict[str, Any]:
...
async def _async_execute_hard_purge(document_id: str, tenant_id: str) -> Dict[str, Any]:
    ...
    async with session_factory() as session:
        ...
        logger.info("Completed background Phase 2 hard purge task", document_id=document_id, vectors=summary.qdrant_points_deleted)
