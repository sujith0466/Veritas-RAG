...
class ScanType(StrEnum):
    ...
    STALE_DETECTOR = "STALE_DETECTOR"  # Detect chunks with drifted embedding model configs
...
class HealthScanRequestDTO(BaseModel):
    ...
    scan_type: ScanType = Field(default=ScanType.ALL, description="Type of health check to run.")
...
class HealthScanJobDTO(BaseModel):
    ...
    orphans_found: int = Field(default=0, description="Total orphaned items identified.")
    orphans_purged: int = Field(default=0, description="Total orphaned items successfully cleaned up.")
    stale_chunks_found: int = Field(default=0, description="Total stale chunks identified needing re-embedding.")
    parity_status: str = Field(default="UNKNOWN", description="Parity check status string (e.g. 'SYNCED (100 == 100)').")
    duration_ms: float = Field(default=0.0, description="Total execution duration in milliseconds.")
    error_message: Optional[str] = Field(default=None, description="Error explanation if job failed.")
...
class ParityAuditDTO(BaseModel):
    ...
    tenant_id: str = Field(..., description="Tenant namespace ID.")
    pg_chunk_count: int = Field(..., description="Active embedded chunks in PostgreSQL.")
    qdrant_point_count: int = Field(..., description="Total points indexed in Qdrant collection.")
...
class PurgeSummaryDTO(BaseModel):
    ...
    tenant_id: str = Field(..., description="Tenant namespace ID.")
    qdrant_points_deleted: int = Field(..., description="Count of points removed from Qdrant vector store.")
    pg_chunks_deleted: int = Field(..., description="Count of chunk rows removed from PostgreSQL.")
    is_fully_purged: bool = Field(..., description="True if both phases completed successfully.")
...
class ModelRotationRequestDTO(BaseModel):
    ...
    new_provider: str = Field(..., description="Target embedding provider (`e.g., 'cohere'`).")
    new_model: str = Field(..., description="Target embedding model (`e.g., 'embed-english-v3.0'`).")
...
class MigrationJobDTO(BaseModel):
    ...
    stale_chunks_enqueued: int = Field(..., description="Total stale chunks enqueued for re-embedding.")
