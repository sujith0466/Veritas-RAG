...
class CandidatePointDTO(BaseModel):
    ...
    rank: int = Field(..., ge=1, description="1-indexed rank position in source candidate list.")
...
class RankedEvidenceDTO(BaseModel):
    ...
    rerank_score: float | None = Field(
        default=None, description="Cross-encoder semantic reranker score (`None` if bypassed)."
    )
    final_rank: int = Field(..., ge=1, description="Final 1-indexed ordering after reranking.")
    matched_sources: list[str] = Field(
        default_factory=list, description="Sources that discovered this chunk (e.g. `['dense', 'sparse']`)."
    )
    metadata: dict[str, Any] = Field(
...
class RetrievalResultDTOv2(BaseModel):
    ...
    dense_candidates_count: int = Field(..., description="Candidates returned from dense search.")
    sparse_candidates_count: int = Field(..., description="Candidates returned from sparse search.")
    unique_candidates_merged: int = Field(..., description="Unique candidates after RRF union and deduplication.")
...
class RetrievalQueryLogDTO(BaseModel):
    ...
    query_text: str = Field(..., description="Executed query string.")
    dense_candidate_count: int = Field(..., description="Number of dense candidates retrieved.")
    sparse_candidate_count: int = Field(..., description="Number of sparse candidates retrieved.")
    merged_unique_count: int = Field(..., description="Unique candidates after fusion/dedup.")
    final_top_k: int = Field(..., description="Number of final evidence items returned.")
...
class RetrievalMetricsDTO(BaseModel):
    ...
    tenant_id: str = Field(..., description="Tenant namespace ID.")
    total_queries_executed: int = Field(default=0, description="Total search queries run.")
    avg_total_duration_ms: float = Field(default=0.0, description="Average total query latency.")
    p95_total_duration_ms: float = Field(default=0.0, description="P95 execution latency.")
    avg_dense_candidates: float = Field(default=0.0, description="Average dense candidates returned.")
    avg_sparse_candidates: float = Field(default=0.0, description="Average sparse candidates returned.")
    avg_merged_candidates: float = Field(default=0.0, description="Average unique merged candidates.")
