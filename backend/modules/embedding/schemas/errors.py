...
class ErrorSeverity(StrEnum):
    ...
    FATAL = (
        "FATAL"  # Permanent quota/auth/input error; transition immediately to FAILED
    )
...
class EmbeddingErrorCode(StrEnum):
    ...
    EMB_004 = (
        "EMB_004"  # Provider network timeout or upstream 5xx failure — RECOVERABLE
    )
...
class EmbeddingDomainException(RAGuardException):
    ...
    def __init__(
        ...
        code_str = (
            code
            if isinstance(code, EmbeddingErrorCode)
            else (
                EmbeddingErrorCode(code)
                if code in EmbeddingErrorCode._value2member_map_
                else str(code)
            )
        )
        self.code = code_str
...
class InvalidInputError(EmbeddingDomainException):
    ...
    def __init__(self, message: str, detail: dict[str, Any] | None = None) -> None:
        super().__init__(
            code=EmbeddingErrorCode.EMB_001, message=message, detail=detail
        )
...
class TokenQuotaExceededError(EmbeddingDomainException):
    ...
    def __init__(self, message: str, detail: dict[str, Any] | None = None) -> None:
        super().__init__(
            code=EmbeddingErrorCode.EMB_002, message=message, detail=detail
        )
...
class RateLimitExceededError(EmbeddingDomainException):
    ...
    def __init__(self, message: str, detail: dict[str, Any] | None = None) -> None:
        super().__init__(
            message=message,
            code=EmbeddingErrorCode.EMB_003,
...
class ProviderTimeoutError(EmbeddingDomainException):
    ...
    def __init__(self, message: str, detail: dict[str, Any] | None = None) -> None:
        super().__init__(
            code=EmbeddingErrorCode.EMB_004, message=message, detail=detail
        )
...
class ProviderAuthenticationError(EmbeddingDomainException):
    ...
    def __init__(self, message: str, detail: dict[str, Any] | None = None) -> None:
        super().__init__(
            code=EmbeddingErrorCode.EMB_005, message=message, detail=detail
        )
