...
class EmbeddingService:
    ...
    async def initiate_embedding_job(
        ...
        chunks = await self.repository.get_unembedded_chunks(tenant_id, document_version_id, batch_size=1000)
        if not chunks and not force_reembed:
            raise InvalidInputError("No unindexed chunks found for the specified document version.")
        ...
        if self.event_dispatcher:
            payload = create_embedding_event(
                ...
                data={"provider": resolved_provider, "model": manager.model_name, "total_chunks": len(chunks)},
            )
            await self.event_dispatcher.publish(
                EmbeddingDomainEvent(event_type=EventType.EMBEDDING_STARTED, payload=payload)
            )
    ...
    async def process_embedding_batch(
        ...
        if not job:
            raise InvalidInputError(f"Embedding job {job_id} not found under tenant {tenant_id}.")

        await self.repository.update_job_progress(job_id, tenant_id, status="PROCESSING")
        job = (await self.repository.get_job_by_id_and_tenant(job_id, tenant_id)) or job

        manager = EmbeddingManager(provider_name=job.provider, model_name=job.model_name)

        while True:
            chunks = await self.repository.get_unembedded_chunks(tenant_id, job.document_version_id, batch_size=batch_size)
            if not chunks:
                break
            ...
            if cached_chunks:
                await self.repository.mark_chunks_as_embedded([c.id for c in cached_chunks], tenant_id)
                await self.repository.update_job_progress(job_id, tenant_id, processed_delta=len(cached_chunks))
            ...
            if missing_chunks:
                ...
                try:
                    ...
                    if self.event_dispatcher:
                        ...
                        await self.event_dispatcher.publish(
                            EmbeddingDomainEvent(event_type=EventType.EMBEDDING_FAILED, payload=payload)
                        )
                    logger.error("embedding_batch_failed", job_id=job_id, error=str(exc))
                    raise
                ...
                await self.repository.mark_chunks_as_embedded([c.id for c in missing_chunks], tenant_id)
                await self.repository.update_job_progress(
                    job_id,
                    tenant_id,
            ...
            if self.event_dispatcher and job:
                ...
                await self.event_dispatcher.publish(
                    EmbeddingDomainEvent(event_type=EventType.EMBEDDING_PROGRESS, payload=payload)
                )

        completed_job = await self.repository.update_job_progress(job_id, tenant_id, status="COMPLETED")
        ...
        if self.event_dispatcher and final_job:
            ...
            await self.event_dispatcher.publish(
                EmbeddingDomainEvent(event_type=EventType.EMBEDDING_COMPLETED, payload=payload)
            )

        logger.info("embedding_job_completed", job_id=job_id, tenant_id=tenant_id)
        return final_job

    async def get_job_status(self, job_id: uuid.UUID, tenant_id: str) -> EmbeddingJob | None:
