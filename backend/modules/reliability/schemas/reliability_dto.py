...
class SearchOptionsDTO(BaseModel):
    ...
    top_k: int = Field(
        default=10, ge=1, le=100, description="Number of candidates to return"
    )
    sla_budget_ms: float = Field(
        default=400.0,
        ge=50.0,
        le=5000.0,
        description="Max allowed execution latency before SLA breach",
    )
    enable_fallback: bool = Field(
        default=True,
        description="Whether to route to degraded BM25 path when circuit trips or times out",
    )
    enable_zero_result_recovery: bool = Field(
        default=True,
        description="Whether to trigger keyword broadening when initial query yields 0 results",
    )
...
class ReliableCandidateDTO(BaseModel):
    ...
    score: float = Field(
        default=0.0, description="Final score (rerank or sparse fallback score)"
    )
    rank: int = Field(default=1, ge=1, description="Final rank position")
    source: str = Field(
        default="hybrid",
        description="Source path (e.g., hybrid, fallback_bm25, zero_broadened)",
    )
    is_fallback: bool = Field(
        default=False, description="True if candidate came from degraded fallback path"
    )
    is_broadened: bool = Field(
        default=False, description="True if candidate came from zero-result recovery"
    )
...
class ReliableRetrievalResultDTO(BaseModel):
    ...
    candidates: list[ReliableCandidateDTO] = Field(
        default_factory=list, description="Ranked candidates"
    )
    duration_ms: float = Field(
        default=0.0, ge=0.0, description="Total execution latency in milliseconds"
    )
    is_sla_breached: bool = Field(
        default=False, description="True if duration_ms exceeded SLA budget"
    )
    is_degraded_fallback: bool = Field(
        default=False,
        description="True if circuit breaker or timeout routed request to fallback path",
    )
    fallback_reason: str | None = Field(
        default=None, description="Reason for degraded fallback route"
    )
...
class CircuitBreakerStateDTO(BaseModel):
    ...
    tenant_id: str = Field(..., description="Tenant namespace")
    target: str = Field(
        ..., description="Target service identifier (e.g., qdrant_hybrid)"
    )
    state: CircuitState = Field(
        default=CircuitState.CLOSED, description="Current circuit state"
    )
    failures: int = Field(
        default=0, ge=0, description="Consecutive failure count in current window"
    )
    last_failure_timestamp: float | None = Field(
        default=None, description="Unix timestamp of last failure"
    )
    cooldown_ttl_seconds: int = Field(
        default=0, ge=0, description="Remaining seconds until HALF_OPEN transition"
    )
...
class SLASummaryDTO(BaseModel):
    ...
    total_queries: int = Field(
        default=0, ge=0, description="Total retrieval queries processed"
    )
    breached_queries: int = Field(
        default=0, ge=0, description="Number of queries exceeding SLA budget"
    )
    degraded_queries: int = Field(
        default=0, ge=0, description="Number of queries served via degraded fallback"
    )
    sla_compliance_rate: float = Field(
        default=100.0, ge=0.0, le=100.0, description="Percentage of queries meeting SLA"
    )
    p95_latency_ms: float = Field(
        default=0.0, ge=0.0, description="Estimated 95th percentile query latency in ms"
    )
