...
class RetrievalFallbackTriggeredPayload(BaseModel):
    ...
    source_module: str = Field(
        default="backend.modules.reliability",
        description="Source domain module package path",
    )
    query_text: str = Field(description="Executed search query text")
    fallback_reason: str = Field(
        description="Reason that triggered fallback (e.g., QdrantTimeout, CircuitBreakerOpen)"
    )
...
class CircuitBreakerTrippedPayload(BaseModel):
    ...
    target_module: str = Field(
        description="Target service module (e.g., qdrant_hybrid)"
    )
    failures: int = Field(description="Consecutive failure count triggering the trip")
    error_code: str | None = Field(
        default=None, description="Error code that caused the trip"
    )
