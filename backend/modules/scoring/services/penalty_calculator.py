...
class PenaltyCalculator:
    ...
    def calculate_penalty(self, inputs: ScoringInputsDTO) -> tuple[float, dict]:
        ...
        unsupported_deduction = (
            inputs.unsupported_claim_count * self.penalty_unsupported_claim
        )
        invalid_cit_deduction = (
            inputs.invalid_citation_count * self.penalty_invalid_citation
        )

        total_deduction = min(
            unsupported_deduction + invalid_cit_deduction, self.max_penalty
        )
