    ...
    def __init__(self) -> None:
        self.default_rules: list[RetryRuleDTO] = [
            RetryRuleDTO(
                ...
                max_attempts_for_rule=3,
            ),
            RetryRuleDTO(
                reason=RetryReason.LLM_API_ERROR,
                action=RetryAction.RETRY_WITH_BACKOFF,
                base_backoff_ms=500,
                max_attempts_for_rule=2,
            ),
            RetryRuleDTO(
                reason=RetryReason.LOW_CONFIDENCE,
                action=RetryAction.RETRY_WITH_REWRITE,
                base_backoff_ms=0,
                max_attempts_for_rule=2,
            ),
            RetryRuleDTO(
                reason=RetryReason.TIMEOUT,
                action=RetryAction.RETRY_WITH_FALLBACK_MODEL,
                base_backoff_ms=200,
                max_attempts_for_rule=1,
            ),
            RetryRuleDTO(
                reason=RetryReason.MALFORMED_OUTPUT,
                action=RetryAction.RETRY_IMMEDIATE,
                base_backoff_ms=0,
                max_attempts_for_rule=1,
            ),
        ]
